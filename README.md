...
3. Depending on user input, the site either:
   ...
   - Runs a full **Recalibration** sequence (jokes, cursor path, flashbang, etc.). :contentReference[oaicite:1]{index=1}  
4. A **STATUS UPDATE** screen appears with fake machine info, then three columns of scrolling “UNBOUND SET” C++-style code. Eventually the terminal melts down into a glitchy transport + glyph flood. :contentReference[oaicite:2]{index=2}  
...
- **`index.html`**  
  ...
  - Loads `styles.css`, `boot-sequence.js`, `status-update.js`, and `home-page.js`. :contentReference[oaicite:5]{index=5}  

- **`styles.css`**  
  - Global styling (VCR OSD Mono font, CRT text, caret, reduced-motion handling).  
  - Boot gate overlay, **transport banner**, **white flood**, and **main hub title** styles. :contentReference[oaicite:6]{index=6}  
...
- **`boot-sequence.js`**  
  - Core boot config (typing speeds, volumes, “last programmed” date detection from file headers). :contentReference[oaicite:8]{index=8}  
  ...
  - Boot gate handler (`setupBootGate`) that:
    ...
    - Then kicks off `runBootSequence()`. :contentReference[oaicite:12]{index=12}  

- **`status-update.js`**  
  - Clears CRT and prints **STATUS UPDATE** with:
    - Fake MACHINE ID (mapped from browser + OS),
    - Random public-looking IP,
    - CURRENT OBJECTIVE: `GENERATE THE MAIN HUB`. :contentReference[oaicite:13]{index=13}  
  - Builds **three code columns** with scrolling C++-style “null_tunnel” / `UNBOUND SET` code, including simple syntax highlighting or plain-text mode depending on version. :contentReference[oaicite:14]{index=14}  
...
- **`home-page.js`**  
  ...
  - **Home node**: lore logbook, design philosophy, and “what this hub is for” copy. :contentReference[oaicite:19]{index=19}  

  - **About node**: profile, capabilities, education, contact section (panels like “Profile”, “Capabilities”, etc.). :contentReference[oaicite:20]{index=20}  
  ...
  - **Footer**: “UNBOUND SET // PERSONAL HUB OF KASI ELLIS” with the current year injected at runtime. :contentReference[oaicite:25]{index=25}  
...
2. Open **`index.html`** in a modern browser (Chrome, Edge, Firefox, Opera, Safari). :contentReference[oaicite:26]{index=26}  
...
> **Note:** All audio requires user interaction due to browser autoplay rules. The boot gate keypress and pointer/keydown events unlock the typing SFX/audio pools. :contentReference[oaicite:28]{index=28}  
...
- **Gallery:** Click any thumbnail to open a full-size image in a new tab. :contentReference[oaicite:32]{index=32}  
- **Playlist:** Hit play inside the embedded YouTube playlist; hub music volume slider only affects the local `home-theme` audio, not YouTube. :contentReference[oaicite:33]{index=33}  
...
- Honors `prefers-reduced-motion`:
  - Disables typing animations and per-char SFX when the user prefers reduced motion. :contentReference[oaicite:35]{index=35}  
...
- **Assets:**  
  - Replace audio files in `assets/` (boot, whoosh, typing, flash, error, hub themes) with your own while keeping IDs in `index.html`. :contentReference[oaicite:37]{index=37}  
- **Themes:**  
...
- **Concept, writing, and hub content:** Kasi Ellis / UNBOUND SET. :contentReference[oaicite:41]{index=41}  
